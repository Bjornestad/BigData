...
print("\nStep 2: Loading station metadata...")
station_metadata = spark.read.parquet('hdfs://namenode:9000/user/hive/warehouse/station_metadata')
station_metadata = station_metadata.select('station_id', 'dk_area')
print(f"  ✓ Loaded {station_metadata.count()} stations")
...
obs_df = spark.sql("""
    SELECT station_id, parameter_id, value, year, month, day, hour
    FROM weather_observations_historical
    WHERE year >= 2021
    AND parameter_id IN (
